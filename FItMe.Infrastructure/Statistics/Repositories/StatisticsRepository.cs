namespace FitMe.Infrastructure.Statistics.Repositories
{
    ...
    internal class StatisticsRepository : DataRepository<IStatisticsDbContext, Statistics>, IStatisticsRepository
    {
        ...
        public async Task IncrementExercises(CancellationToken cancellationToken = default)
        {
            var statistics = await this.Data
                .Statistics
                .SingleOrDefaultAsync(cancellationToken);

            statistics.AddExercise();

            await this.Save(statistics, cancellationToken);
        }

        public async Task<int> GetExerciseViews(int exerciseId, CancellationToken cancellationToken = default)
            => await this.Data
                    .ExerciseViews
                    .CountAsync(cav => cav.ExerciseId == exerciseId, cancellationToken);
    }
}
